...
- `docs/00_Project/`：專案背景、總覽、地圖與連結。
- `docs/01_Architecture/`：平台架構、UI 基準、版面與核心架構。
- `docs/02_Modules/`：各中心的模組責任與初步規劃。
- `docs/03_Project/`：Backlog、Roadmap 與 Sprint 規劃。
- `docs/04_AI/`：AI Agent 上手文件、規則與輸出模板。
- `assignments/`：任務分派與工作包入口。
- `templates/`：可重用文件模板入口。
- `references/`：外部參考、決策依據與補充資料入口。
